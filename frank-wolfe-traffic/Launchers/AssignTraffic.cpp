...
void assignTraffic(const CommandLineParser& clp) {
	...
	Graph graph(infilename, ceParameter, constParameter); //this is the entry point for python
	...
	std::vector<ClusteredOriginDestination> odPairs = importClusteredODPairsFrom(odFilename); //this
	...
	FrankWolfeAssignmentT assign(graph, odPairs, csv, patternFile, pathFile, weightFile, clp.isSet("v"), clp.isSet("elastic")); //also this
	...
	assign.run(numIterations); //and this
	std::cout << "Total time:," << timer.elapsed() << std::flush;
